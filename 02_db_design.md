...
本システムでは、B-Treeインデックスを基本とする。
B-treeインデックスが効果を発揮するのはレコードの数が  

※この続きからDB設計の本でインデックスの利用条件
※この続きから
※この続きから
※この続きから
※この続きから
...
本システムは実際のレコードはそこまで到達しないが、想定は運用していけば10万件以上は増える想定のため、B-treeインデックスを使用する。
検索条件、認可判定、一覧表示、売上集計、バッチ処理で使用頻度が高いカラムにインデックスを設定する。
主キー、外部キー、UNIQUE制約により自動的に作成されるインデックスは原則として個別に重複定義しない。
複合インデックスは、WHERE句で同時に使用されるカラムの組み合わせを対象とする。

### 6.2 インデックス一覧

| テーブル              | インデックス対象カラム | 種別   | 目的                                                           |
| --------------------- | ---------------------- | ------ | -------------------------------------------------------------- |
| users                 | keycloak_user_id       | UNIQUE | JWTのsubまたはKeycloakユーザーIDから業務ユーザーを検索するため |
| users                 | store_id               | 通常   | 店舗ごとのスタッフ一覧を取得するため                           |
| restaurant_tables     | qr_token               | UNIQUE | 固定QRトークンから対象テーブルを検索するため                   |
| restaurant_tables     | store_id, status       | 複合   | 店舗ごとのテーブル一覧やステータス別絞り込みを行うため         |
| visit_sessions        | visit_token            | UNIQUE | 来店トークンから来店セッションを取得するため                   |
| visit_sessions        | table_id, status       | 複合   | テーブルに紐づく有効な来店セッションを検索するため             |
| menus                 | store_id, status       | 複合   | 店舗ごとの販売中メニュー一覧を取得するため                     |
...
| payments              | visit_session_id       | UNIQUE | 来店セッション単位の会計情報を取得するため                     |
| payments              | store_id, paid_at      | 複合   | 店舗別・期間別の売上集計を行うため                             |
| daily_sales_summaries | store_id, sales_date   | UNIQUE | 店舗別・日付別の売上集計結果を一意に取得するため               |
| batch_job_histories   | job_name, start_time   | 複合   | バッチ名と実行開始時刻で履歴を検索するため                     |
